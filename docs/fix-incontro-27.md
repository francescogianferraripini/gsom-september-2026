...
- **22 · Dal contratto al protocollo: MCP** — `#slide-22` — `slide15-sequence-giro.svg` + `slide22-mcp-registro.svg` (in transizione)
	...
	* Visual: secondo me il visual con mcp: il server MCP non deve sostituire il registro dei tool, ma aggiungersi a destra del registro, e far vedere che in inizializzazione aggiunge al registro del tool dell'harness i tool esposti dal server MCP.
	  In entrambi i visual il blocco nero lo chiamerei Harness Loop e lo farei vedere con il Registro dei tool dentro un blocco harness
	- ✅ FATTO — rifatti entrambi i tempi, e con loro la figura della 15b che è il tempo 1.
		- **Il blocco harness.** In `slide15-sequence-giro.svg` e in `slide22-mcp-registro.svg` c'è ora un **contenitore nero «harness»** (`238-642`) che racchiude il blocco nero e il registro dei tool. Il blocco nero non si chiama più «harness» ma **«Harness Loop»**. L'etichetta del contenitore sta a cavallo del bordo superiore, fra il loop e il registro, dove non copre nulla.
		- **Il server si aggiunge, non sostituisce.** Nel tempo 2 il registro **resta dentro l'harness, nella stessa posizione del tempo 1**: quello che cambia è il suo contenuto. In cima compare un blocco ocra **«+ 3 tool dal server MCP «ordini» · aggiunti al giro 0, con `tools/list`: l'harness non ne scrive il codice»**, e i tre tool portano l'etichetta «da MCP «ordini»». In fondo al registro: *«i tool nativi dell'harness restano al loro posto: questi si aggiungono.»*
		- **La colonna nuova.** Il server MCP è una colonna a sé (`656-780`) **fuori dal contenitore harness**, con dentro il codice vero delle tre funzioni. L'API ordini si sposta all'estrema destra (`794-838`): in mezzo, ora, c'è il server.
		- **Le frecce.** Al **giro 0** una `tools/list` esce dal registro, raggiunge il server e la risposta (`3 × {name, schema}`) rientra nel registro: è l'inizializzazione, disegnata. Nel **giro 1** il dispatch va al registro come nel tempo 1 (*«il registro sa a chi girarla»*), e solo da lì parte la `tools/call` verso il server, che chiama l'API.
		- *Testo*: il punto «La soluzione» non dice più che MCP «sposta il registro fuori dall'harness», ma che sposta **il codice** dei tool in un server, mentre il registro resta e si popola. Nelle note: un harness ha i suoi tool nativi, e i server gliene aggiungono altri; non è un aut aut.
		- **Misure**: viewBox invariato `840×440` in entrambi, così la dissolvenza fra i due tempi non sposta modello, Harness Loop e registro. I testi che attraversano il bordo del contenitore hanno un alone bianco (`paint-order`), altrimenti la linea li tagliava a metà.
		- ⚠️ Il tempo 1 è la figura della **slide 15b**: il contenitore e il rename si vedono anche lì, come chiedevi («in entrambi i visual»).
...
- **32 · Il prefisso non si tocca: prefix caching** — `#slide-32` — `slide32-prefix-caching.svg`
	- Non mi torna l'altezza del blocco grigio in background sulla cache. Parliamone
	- ✅ FATTO — **grigio sul solo prefisso**, come hai deciso dopo averne parlato.
		- Il rettangolo grigio finiva a `y=226`, cioè **a metà dello spazio vuoto** fra il prefisso (che finisce a 198) e la storia (che comincia a 236): non era allineato a niente. Ora è `84-204`, aderente al solo blocco prefisso.
		- ⚠️ **C'era anche un'incoerenza, e andava sciolta**: l'area grigia diceva «in cache c'è il prefisso», ma la didascalia sotto la finestra n+1 diceva «solo questo si calcola» indicando la riga nuova. Se la cache si ferma al prefisso, anche storia e risultati si ricalcolano. La didascalia ora dice **«riusato il prefisso: si calcola il resto»**, e a fianco della finestra n+1 una graffa burgundy copre storia, risultati e riga nuova con l'etichetta **«si ricalcola»**.
		- Questa lettura è quella coerente con la slide 33: il checkpoint `cache_control` si mette in fondo alla parte condivisa, cioè al prefisso. Detto anche nelle note del relatore.
		- L'etichetta verticale è stata accorciata a **«in cache: K e V»**: quella lunga sfondava sopra il rettangolo e finiva sotto la freccia «riusati».
...
- **43 · Il processo, in un colpo d'occhio** — `#slide-43` — `slide43-processo-eval.svg`
	- rendi evidente (al limite duplicando la barra Il team di sviluppo dell'agente) che a partire dallo step 3 soprattutto si fixano i problemi
	- ✅ FATTO — una **terza fascia**, bordeaux, che parte esattamente sopra la tappa 3 e arriva in fondo: **«da qui in poi si correggono i problemi»**, con l'inciso *«il cluster dice quale leva tirare; checker, giudice e produzione dicono se ha funzionato»*.
		- Le tre fasce ora si leggono come una scala: verde acqua sulle tappe 1-3 (l'esperto), azzurra dalla 4 (il team), bordeaux dalla 3 (la correzione). Il bordeaux è lo stesso della freccia di ritorno in basso, così la fascia e il ciclo «a ogni modifica si rigira tutto» sono riconoscibili come la stessa storia.
		- ⚠️ Ho preferito una fascia nuova al duplicato di quella azzurra: le prime due dicono **chi** lavora, questa dice **che cosa** si sta facendo. Duplicare l'azzurra avrebbe fatto sembrare la tappa 3 condivisa fra i due ruoli come la 5, che non è il punto.
		- ⚠️ Il viewBox non cambia (1140×360, già alto quanto lo slot: la figura è vincolata in altezza). I 36 px della fascia nuova vengono dal margine sopra e dagli spazi fra le fasce, che si stringono; le sei tappe, la freccia di ritorno e la fascia Observability restano dov'erano. È uscita la scritta *«↓ dove i due si incontrano»* sotto la tappa 5: lo dicono già il doppio bordo attorno alla tappa e il chip verde acqua sopra («rientra: valida il giudice»).
